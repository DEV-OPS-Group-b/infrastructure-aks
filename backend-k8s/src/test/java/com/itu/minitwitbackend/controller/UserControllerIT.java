...
public class UserControllerIT {
    ...
    void follow_User_Success() {
        // arrange
        var currentUser = userRepository.save(testUser);
        var target = UserEntity.builder().username("new target").build();
        var targetUser = userRepository.save(target);
        var user = new FollowUserRequest(testUser.getUsername(),target.getUsername());

        // act
        ResponseEntity<String> responseEntity = testRestTemplate.postForEntity(
                "/devops/user/follow", user, String.class);

        var user2 = userRepository.findById(currentUser.getId());
...
//    }
    ...
    void unfollow_User_Success() {
        ...
        var target = UserEntity.builder().username("new target").build();
        var followingList = new ArrayList<String>(){{
                add(target.getUsername());
        }};
        current.setFollowing(followingList);

        var currentUser = userRepository.save(current);
        var targetUser = userRepository.save(target);
        var user = new FollowUserRequest(currentUser.getUsername(),targetUser.getUsername());
    ...
    void follow_validate_isFollowing_Success() {
        ...
        var firstUser = new FollowUserRequest(currentUser.getUsername(),targetUser.getUsername());
        var secondUser = new FollowUserRequest(targetUser.getUsername(),currentUser.getUsername());
        ...
        var userFromDb = userRepository.findById(currentUser.getId());
        var user2FromDb = userRepository.findById(targetUser.getId());
        ...
        assertThat(userFromDb.get().getFollowing().size()).isEqualTo(1);
        assertThat(user2FromDb.get().getFollowing().size()).isEqualTo(1);
       // ------- is following setup-------
